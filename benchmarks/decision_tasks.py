...
as two verbalizer sets — multiple surface realizations per outcome
(" Approved", " Approve", " Accept", " Yes") rather than one fixed token. This is
the generalization the V5 decision functional enables.

Multi-token decision words are supported. analyze() resolves each variant to a
representative token for last-position attribution; the runner's ``--sequence``
mode scores the full multi-token set via teacher forcing
(decision.value_from_scores). Variants need NOT be single tokens.
...
# ---------------------------------------------------------------------------
# The suite. Five Annex III decision shapes, each with rich (multi-token)
# verbalizer sets — the real point of the V5 decision functional.
# ---------------------------------------------------------------------------
DECISION_TASKS: List[DecisionTask] = [
    DecisionTask(
        ...
        positive_variants=(" Approved", " Approve", " Accept", " Yes"),
        negative_label="deny",
        negative_variants=(" Denied", " Deny", " Reject", " No"),
        expected="positive",
        annex_iii_ref="5(b)",
    ),
    DecisionTask(
