...
class SlackNotifier:
    ...
    def send_audit_complete(
        ...
        blocks.append({
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": f"Glassbox v3.3.0 · {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())} · Regulation (EU) 2024/1689"}],
        })
    ...
    def send_compliance_drop(
        ...
        blocks.append({
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": f"Glassbox v3.3.0 · Article 72 Regulation (EU) 2024/1689 · {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}"}],
        })
        return self._send(blocks=blocks)
    ...
    def send_circuit_drift(
        ...
        blocks.append({
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": f"Glassbox CircuitDiff v3.3.0 · Article 72 Regulation (EU) 2024/1689 · {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}"}],
        })
        return self._send(blocks=blocks)
    ...
    def _build_card(
        ...
        card: Dict[str, Any] = {
            ...
            "sections": [
                {
                    "facts": facts,
                    "text":  f"Generated by Glassbox AI v3.3.0 · {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}",
                }
            ],
        }
        if report_url:
